...
def add_data():
    try:
        ...
        add_values = document_ref.document().create(dict(temperature=temperature, humidity=humidity, altitude=0, pressure=0, date=datetime.datetime.utcnow()))
        return jsonify({
            "status": "success"
        }), 201
...
if __name__ == '__main__':
    #app.run(debug=True)
    app.run(host='10.0.0.32', port=5000)